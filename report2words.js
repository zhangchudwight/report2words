let apMetar =
	"METAR ZGSZ 111200Z 36008G12MPS 200V360 8000 TSRA BR SCT013 BKN040 17/15 Q1015 RETSRA BECMG AT0735 12008MPS 0800 BR FG GR +TSRA FEW015 SCT020 BKN025CB=";
let apTaf =
...
function report2words(report) {
	...
	if (/[AMDCOR]{3}/.test(rawReport[n])) {
		let timewords = rawReport[n + 2].substr(0, 2) + "日" + rawReport[n + 2].substr(2, 2) + "时" + rawReport[n + 2].substr(4, 2) + "分UTC"
		words[0] = rawReport[n + 1] + timewords + "发布的" + tafvalidtime + words[0] + dict[rawReport[n]];

	} else {
		let timewords = rawReport[n + 1].substr(0, 2) + "日" + rawReport[n + 1].substr(2, 2) + "时" + rawReport[n + 1].substr(4, 2) + "分UTC"
		words[0] = rawReport[n] + timewords + "发布的" + tafvalidtime + words[0];
				...
				if (/TAF/.test(report)) {
					switch (true) {
						case /BECMG/.test(rawReport[n]):
							{
								words.push("预计在" + Number(rawReport[n + 1].substr(0, 2)) + "时——" + Number(rawReport[n + 1].substr(2, 2)) + "时发生渐变:");
								n = n + 2;
								break;
							}
						case /TEMPO/.test(rawReport[n]):
							{
								words.push("预计在" + Number(rawReport[n + 1].substr(0, 2)) + "时——" + Number(rawReport[n + 1].substr(2, 2)) + "时短时出现:");
								n = n + 2;
								break;
							}
						case /FM/.test(rawReport[n]):
							{
								words.push("预计在" + Number(rawReport[n + 1].substr(0, 2)) + "时" + Number(rawReport[n + 1].substr(2, 2)) + "分发生变化:");
								n = n + 2;
								break;
							}
					}
						...
						case /BECMG/.test(rawReport[n]):
							{
								...
								if (/FM/.test(rawReport[n + 1])) {
									words[l] = words[l] + "从" + Number(rawReport[n + 1].substr(2, 2)) + "时" + Number(rawReport[n + 1].substr(4, 2)) + "分";
								}
								if (/AT/.test(rawReport[n + 1])) {
									words[l] = words[l] + "在" + Number(rawReport[n + 1].substr(2, 2)) + "时" + Number(rawReport[n + 1].substr(4, 2)) + "分";
								}
								if (/TL/.test(rawReport[n + 1])) {
									words[l] = words[l] + "到" + Number(rawReport[n + 1].substr(2, 2)) + "时" + Number(rawReport[n + 1].substr(4, 2)) + "分";
								}
								words[l] = words[l] + "出现:";
								n = n + 2;
								break;
...
function element2words(type, code) {
	switch (type) {
		...
		case "cloud":
			{
				...
				if (code === "/////") {
					...
					cloud = cloud + dict[/[A-Z]+/.exec(code)[0]] + ",云高" + Number(/\d+/.exec(code)[0]) * 30 + "米";
					if (/CB/.test(code)) {
						cloud = cloud + ",积雨云";
					}
				}
		...
		case "re":
			{
				let weather = code.substr(2, code.length - 1);
				let w = "";
